...
document.getElementById('inputButtonPrice').addEventListener('click', () => {
    processSearchByPrice(document.getElementById('inputPrice').value, 50);
});
...
api.searchProductsByType().then(function (value) {
    updateTable('allTable',value);
});

api.searchProductsByPrice().then(function (value) {
    updateTable('allTable',value);
});
...
function processSearchType(searchType) {
    ...
    .then(function (val) {
        var emptyExamined = { id: '', type: '', price: '' };
        updateExaminedText(emptyExamined);
...
function processSearchByPrice(price, difference) {
    api.searchProductsByPrice(price, difference)
        .then(val => {
        let emptyExamined = { id: '', type: '', price: '' };
    updateExaminedText(emptyExamined);
    updateTable('similarTable', val);
})
.catch(val => {
